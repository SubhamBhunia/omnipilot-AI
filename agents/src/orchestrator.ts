import { CropSoilAgent, SoilRecommendation } from "./crop_soil.js";
import { PestDiseaseAgent, DiseaseRecommendation } from "./pest_disease.js";
import { SafetyEnforcerAgent, SafetyAuditResult } from "./safety_enforcer.js";
...
export interface OrchestratorOutput {
  ...
  zoneId: string;
  cropType: string;
  cropSoilAdvice?: SoilRecommendation;
  pestDiseaseAdvice?: DiseaseRecommendation;
  safetyAudit: SafetyAuditResult;
  finalAnswer: string;
  trace: AgentFlowTrace[];
}

export class AgriOrchestratorAgent {
  private name = "AgriOrchestratorAgent";
  private systemPrompt = `You are the primary AgriSustain Orchestrator. 
...
5. Provide a consolidated, safe response.`;
  ...
  private cropSoilAgent = new CropSoilAgent();
  private pestDiseaseAgent = new PestDiseaseAgent();
  private safetyEnforcer = new SafetyEnforcerAgent();
  ...
  public async runWorkflow(
    ...
    let cropSoilAdvice: SoilRecommendation | undefined;
    let pestDiseaseAdvice: DiseaseRecommendation | undefined;
    ...
    if (normalizedQuery.includes("pest") || normalizedQuery.includes("disease") || normalizedQuery.includes("yellow") || normalizedQuery.includes("spot") || normalizedQuery.includes("bug") || normalizedQuery.includes("mildew") || normalizedQuery.includes("pesticide") || normalizedQuery.includes("infestation")) {
      ...
      if (normalizedQuery.includes("yellow")) symptoms.push("yellow spots");
      if (normalizedQuery.includes("mildew")) symptoms.push("powdery mildew");
      if (normalizedQuery.includes("bug") || normalizedQuery.includes("aphid")) symptoms.push("aphids");
    ...
    let proposedDosage: number | undefined;
